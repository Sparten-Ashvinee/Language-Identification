...
#k-means
#print ('Working on KNN training... ', flush=True)
KNNclssifier = KNeighborsClassifier()
KNNclssifier.fit(trainset, tags)

#print ('Working on KNN prediction... ', flush=True)
devset = ngram_vectorizer.transform(X_dev)
predictions = KNNclssifier.predict(devset)
...
y_train_pred = cross_val_predict(KNNclssifier, trainset, tags,cv=3)
...
for line in pf:
    ...
    predvalue = KNNclssifier.predict(strmod)
    print(predvalue)
